...
export default function MeetingCard({ meeting, refresh= ()=>{} }) {

  const {user} = useStore();
  const [open, setOpen] = useState(true);
  const deleteMeeting = (id, e) => {
    e.preventDefault();
    Axios.delete(`/meetings/${id}`).then(async() => {
      ...
      toast.error("Une erreur s'est produite lors de la suppression de la réunion.")
      console.log(err);
    })
  };
  const MONTHS = [
      ...
      <div className="flex  items-center ">
        ...
        <div className="basis-3/4">
          ...
          {/* <button
            ...
            className=" w-fit flex items-center gap-1 text-gray-400 hover:text-gray-500 "
          >
            voir plus{" "}
            <MdOutlineKeyboardDoubleArrowDown
          ...
          {open && (
            <div className="transition-all mt-2 ">
              ...
              <p className="py-1 pl-2">{meeting?.description}</p>
              <p className="capitalize text-gray-700 font-bold ">Pv: </p>
              {meeting?.pv ? (
                meeting?.pv
              ) : (
